...
bed_file_path = 'C:\\Users\\hoffmannmd\\OneDrive - National Institutes of Health\\00_PROJECTS\\GAS_motifs_WGS\\02_d_AlmostGASmotifs__samples_annotated_immuneGenes\\annotated_others.bed'

# Read the DESeq2 normalized matrix and the BED file
deseq2_matrix = pd.read_csv(deseq2_matrix_path, sep="\t", index_col=0)
bed_file = pd.read_csv(bed_file_path, sep="\t", header=None)
...
def create_boxplots_and_tests(deseq2_matrix, bed_file):
    ...
    for _, row in bed_file.iterrows():
        ...
        vcf_samples = [sample.split("_")[1] for sample in vcf_samples]  # Extract sample IDs (LUDx)
        ...
        for gene in genes:
            ...
            title = f"{gene} ({row['sequence']}) - p-value: {'NA' if p_value is None else f'{p_value:.3e}'}"
            plt.title(title)
...
results = create_boxplots_and_tests(deseq2_matrix, bed_file)

# Save results to a CSV file
results.to_csv("boxplot_statistical_results.csv", index=False)
